<template>
  <div class="right_main_under">
    ...
    <el-row>
      <el-col :span="24">
        <div class="grid-content bg-purple">
          <el-button type="primary" size="mini" plain class="left" @click="add">添加</el-button>
        </div>
      </el-col>
    </el-row>
    <el-row style="margin-top: 20px">
      <el-col :span="24">
        <el-table border ref="multipleTable" :data="tableData" style="width: 100%" v-loading="loading" max-height="550px"
          size="mini" class="tableStyle" empty-text="暂无数据">
          ...
          <el-table-column label="操作">
            ...
            <template slot-scope="scope">
              <el-button size="mini" type="text" @click="xiugai(scope.row)">修改</el-button>
        ...
        <div class="bottom">
          <div class="ss">
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
              :current-page="mypageable.pageNum" :page-sizes="[10, 20, 30, 40]" :page-size="mypageable.pageSize"
              layout="total, sizes, prev, pager, next, jumper" :total="total" class="pagePagination pageRight">
            </el-pagination>
          </div>
        </div>
      </el-col>
    ...
    <!-- 新增 -->
    <el-dialog title="新增" :visible.sync="dialogVisible" width="35%" :before-close="handleClose" class="dialogInfo"
      :close-on-click-modal="true">
      <el-form ref="newdomainSimpleVo" label-width="80px" :inline="true" :model="newdomainSimpleVo" size="mini">
        <!-- 用户名 -->
        <el-form-item label="用户名：" prop="user">
          <el-input v-model="newdomainSimpleVo.user" placeholder="请输入用户名"></el-input>
        </el-form-item>
        <!-- 密码 -->
        <el-form-item label="密码：" prop="pwd">
          <el-input v-model="newdomainSimpleVo.pwd" placeholder="请输入密码"></el-input>
        </el-form-item>
        <el-form-item label="角色:">
          <el-select v-model="newdomainSimpleVo.role" placeholder="请选择角色">
            <el-option v-for="(value, i) in newdomainSimpleVo.roleshuzu" :key="i" :value="i" :label="value">{{ value
            }}</el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="状态">
          ...
          <el-radio v-model="newdomainSimpleVo.radio" label="0">未激活</el-radio>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="queding" plain>确 定</el-button>
        <el-button @click="quxiao" size="mini" plain>取 消</el-button>
      </span>
    </el-dialog>
    <!-- 修改 -->
    <el-dialog title="修改" :visible.sync="dialogVisible1" width="35%" :before-close="handleClose1" class="dialogInfo"
      :close-on-click-modal="true">
      <el-form ref="newdomainSimpleVo" label-width="80px" :inline="true" :model="newdomainSimpleVoxiugai" size="mini">
        <!-- 用户名 -->
        <el-form-item label="用户名：" prop="user" >
          <el-input v-model="newdomainSimpleVoxiugai.userxiugai" placeholder="请输入用户名" :disabled="true"></el-input>
        </el-form-item>
        <!-- 密码 -->
        <el-form-item label="密码：" prop="pwd">
          <el-input v-model="newdomainSimpleVoxiugai.pwdxiugai" placeholder="请输入密码"></el-input>
        </el-form-item>
        <el-form-item label="角色:">
          <el-select v-model="newdomainSimpleVoxiugai.rolexiugai" placeholder="请选择角色">

            <el-option v-for="(value, i) in newdomainSimpleVoxiugai.roleshuzuxiugai" :key="i" :value="i" :label="value">{{
              value }}</el-option>
          </el-select>
        ...
        <br />
        <el-form-item label="状态:">
          <el-radio v-model="newdomainSimpleVoxiugai.radioxiugai" label="1">激活</el-radio>
          <el-radio v-model="newdomainSimpleVoxiugai.radioxiugai" label="2">未激活</el-radio>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="quedingxiugai" plain>确 定</el-button>
...
export default {
  ...
  methods: {
    ...
    async newlist() {
      ...
      const { data: res } = await this.$http.get("/admin/user/list", { params: list });
      if (res.code == 200) {
